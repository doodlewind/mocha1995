...
#if 0 /* XXX don't do this! it breaks %g and the other float formats! */
/* was: #if defined(XP_PC) && !defined(_WIN32) */
/*
** Win16 cannot use the runtime library version of sprintf(...)  Use
** the one provided by Windows instead.
*/
...
#endif
#else
#define FETCH_LL(d,ap)			 \
{					 \
    ((long*)&(d))[0] = va_arg(ap, long); \
...
*/
static int cvt_ll(SprintfState *ss, int64 num, int width, int prec, int radix,
		  int type, int flags, const char *hexp)
{
    char cvtbuf[100];
    char *cvt;
...
static int FuncStuff(SprintfState *ss, const char *sp, size_t len)
{
    int rv;
...
PR_PUBLIC_API(size_t) PR_sxprintf(PR_sxprintf_callback func, void *arg, 
                                 const char *fmt, ...)
{
    va_list ap;
    int rv;
...
PR_PUBLIC_API(size_t) PR_vsxprintf(PR_vsxprintf_callback func, void *arg, 
...
** before it overflows.
*/
static int GrowStuff(SprintfState *ss, const char *sp, size_t len)
{
    ptrdiff_t off;
    char *newbase;
...
*/
PR_PUBLIC_API(char *) PR_smprintf(const char *fmt, ...)
{
    va_list ap;
    char *rv;
...
PR_PUBLIC_API(char *) PR_vsmprintf(const char *fmt, va_list ap)
{
    SprintfState ss;
    int rv;
...
*/
static int LimitStuff(SprintfState *ss, const char *sp, size_t len)
{
...
** when finished.
*/
PR_PUBLIC_API(size_t) PR_snprintf(char *out, size_t outlen, const char *fmt, ...)
{
    va_list ap;
    int rv;
...
PR_PUBLIC_API(size_t) PR_vsnprintf(char *out, size_t outlen, const char *fmt, 
                                  va_list ap)
{
    SprintfState ss;
    size_t n;
...
PR_PUBLIC_API(char *) PR_sprintf_append(char *last, const char *fmt, ...)
{
    va_list ap;
    char *rv;
...
PR_PUBLIC_API(char *) PR_vsprintf_append(char *last, const char *fmt, va_list ap)
{
    SprintfState ss;
    int rv;
